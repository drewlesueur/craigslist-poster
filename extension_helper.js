...
var ad, agree_to_map, callbacker, check_east_valley, check_housing_offered, check_real_estate_by_broker, cl_home, click_continue, close_cl_tab, create_tab, data_url, details, do_next_posting, done_images, download_image, exec, fill_posting, get_listing_details, go_to_posting, ids, listing, nav, new_posting, search_for_next_ad, toggler, upload_image, wait_for_redirect, wait_til_ad_finishes_loading, zip,
  __slice = [].slice;
...
do_next_posting = function() {
  return setTimeout(do_posting, 1000);
};
...
wait_til_ad_finishes_loading = function() {
  return setTimeout(next_step, 7000);
};
...
close_cl_tab = function() {
  return crome.tabs.remove();
};
