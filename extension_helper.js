...
var ad, agree_to_map, callbacker, check_east_valley, check_housing_offered, check_real_estate_by_broker, click_continue, cragslist_home, craigslist_tab, create_craigslist_tab, create_homeseekr_tab, create_tab, data_url, debug, debug_off, debug_on, details, do_next_posting, do_posting, done_images, download_image, exec, exec_with_args, fifteen_min, fill_posting, get_listing_details, get_next_ad, give_or_take_15, go_to_posting, homeseekr_home, homeseekr_tab, ids, listing, make_2_new_tabs, max_price, nav, new_posting, next_step, nexter, search_for_next_ad, select_craigslist_tab, select_homeseekr_tab, setup_ad_cycle, start_posting, stepper, two_hours, two_minutes, upload_image, wait, wait_for, wait_for_nav, wait_for_redirect, wait_til_ad_finishes_loading, waiting_for_nav, zip,
  __slice = [].slice;
...
do_next_posting = function() {
  return wait(two_hours + give_or_take_15(), function() {
    return do_posting();
  });
};
...
fill_posting = function() {
  return exec(function(details, ad, zip, max_price) {
    ...
    $("span:contains(Posting Description:)").nextAll('textarea').val("<a href=\"http://homeseekr.com/#" + zip + "/" + max_price + "/1\"><img src=\"http://homeseekr.com:8502/cached_image?zip=" + zip + "&max_price=" + max_price + "&page=1\"></a>\n" + details.PublicRemarks);
